/*
京享红包
0 0,10,20,21 * * * jd_nhRedEnvelope.js
添加环境变量JD_FLCODE 如需自己返利，请填写该变量（https://u.jd.com/后面的英文）
*/
const $ = new Env("京享红包");
...
async function main() {
  ...
  $.UA = `jdapp;iPhone;10.3.0;14.6;${randomString(40)};M/5.0;network/wifi;ADID/;model/iPhone8,1;addressid/2308460622;appBuild/167903;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1;`;
  $.max = false;
  $.hotFlag = false;
  const flCodeArr = ["SMIg8Yr"];
...
async function getCoupons(shareCode = '', type = 1) {
  return new Promise((resolve) => {
    ...
    let opts = {
      url: `https://api.m.jd.com/api?functionId=getCoupons&appid=u&_=${Date.now()}&loginType=2&body=${body}&client=apple&clientVersion=8.3.6&h5st=${h5st}`,
      headers: {
        "Accept-Language": "zh-cn",
        "Accept-Encoding": "gzip, deflate, br",
        Cookie: `${cookie} ${$.newCookie}`,
      ...
      try {
        if (err) {
          ...
          if (typeof res == "object") {
            ...
            if (shareCode && typeof res.data !== "undefined" && typeof res.data.joinNum !== "undefined") {
