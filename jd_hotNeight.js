...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        let canHelp = true;
        let helpTime = 0;
        for (let j = 0; j < allShareList.length && canHelp && helpTime < 3; j++) {
            ...
            let assistInfo = await takeRequest(cookie,`party1031_assist`,`{"inviteCode":"${oneCodeInfo.code}"}`);
            if(assistInfo.bizCode === 0){
                console.log(`助力成功`);
                oneCodeInfo.need--;
                helpTime++;
...
async function main(cookie) {
    ...
    console.log(`${userName},助力码：${initInfo.result.inviteCode}`)
    await $.wait(1000);
    if(!initInfo.result.haveReminded){
        console.log(`${userName},去预约`);
        let remindInfo = await takeRequest(cookie,`party1031_remind`);
        console.log(`${userName},预约结果：`+JSON.stringify(remindInfo));
    }
    let matchInfo = {}
    let time = 0;
    do {
        await $.wait(1000);
        console.log(`\n${userName},去匹配`);
        matchInfo = await takeRequest(cookie,`party1031_match`);
        console.log(`${userName},匹配结果：`+JSON.stringify(matchInfo));
        time++
...
async function takeRequest(cookie,functionId,bodyInfo='{}'){
    ...
    let url = `https://api.m.jd.com/client.action?advId=${functionId}`;
    const headers = {
        'Host' : `api.m.jd.com`,
        'Accept' : `application/json`,
        'Origin' : `https://h5static.m.jd.com`,
        'Cookie' : cookie ,
        'Referer' : `https://h5static.m.jd.com/evening-party/index.html`,
