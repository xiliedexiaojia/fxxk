/**
 * Last Modified time: 2021-08-24 22:58:02
 * 东东工厂，不是京喜工厂
 * 活动入口：京东APP首页-京东电器-东东工厂
 ...
 * 环境变量：JDFACTORY_FORBID_ACCOUNT，控制第几个ck不运行此脚本：e.g. JDFACTORY_FORBID_ACCOUNT=4&9&15
 */
const $ = new Env('东东工厂');
...
const inviteCodes = ["T0225KkcR0pI9lDVdRnznKYKJwCjVWnYaS5kRrbA@T0225KkcRRsY8lLfch-hwvIJIgCjVWnYaS5kRrbA",
                     "T0225KkcR0pI9lDVdRnznKYKJwCjVWnYaS5kRrbA@T0225KkcRRsY8lLfch-hwvIJIgCjVWnYaS5kRrb"];
!(async () => {
    await requireConfig();
    if (!cookiesArr[0]) {
    ...
    if ($.taskVos && $.taskVos.length > 0) {
        for (let item of $.taskVos) {
            ...
            if (item.taskType === 27) {
                // 预约商品得电量
                if (item.status === 1) {
                    console.log(`要做这个任务了哟~ ${item.taskName}`);
                    for (let pro of item.productInfoVos) {
                        await jdfactory_collectScore(pro.taskToken);
                    }
                }
            }
            if (item.taskType === 15) {
            ...
            if (item.taskType === 9) {
                ...
                if (item.status === 1) {
                    let num = 0;
                    console.log(`要做这个任务了哟~ ${item.taskName}`);
                    for (let pro of item.shoppingActivityVos) {
                        if (num++ === 9) {
                            break;
                        }
                        await jdfactory_collectScore(pro.taskToken);
