...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      UA = `jdapp;iPhone;10.2.0;14.4.2;${randomString()};network/wifi;model/iPhone10,2;addressid/4091160336;appBuild/167741;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_4_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1`
      await red();
    }
  }
  if (message) {
...
function getCoupons(shareId = '') {
  return new Promise(resolve => {
    let options = {
      url: `https://api.m.jd.com/api?functionId=getCoupons&appid=u&_=${Date.now()}&loginType=2&body=${encodeURIComponent(JSON.stringify({"platform":4,"unionActId":"31134","actId":$.actId,"d":"3wRsPts","unionShareId":shareId,"type":1,"eid":$.eid}))}&client=apple&clientVersion=8.3.6&h5st=undefined`,
      headers: {
        "Host": "api.m.jd.com",
        "Origin": "https://prodev.m.jd.com",
        "User-Agent": UA,
        "Accept": "*/*",
        "Referer": "https://prodev.m.jd.com/mall/active/2GdKXzvywVytLvcJTk2K3pLtDEHq/index.html",
...
function shareUnionCoupon() {
  return new Promise(resolve => {
    let options = {
      url: `https://api.m.jd.com/api?functionId=shareUnionCoupon&appid=u&_=${Date.now()}&loginType=2&body=${encodeURIComponent(JSON.stringify({"unionActId":"31134","actId":$.actId,"platform":4,"unionShareId":$.shareCode,"d":"3wRsPts","supportPic":2,"supportLuckyCode":0,"eid":$.eid}))}&client=apple&clientVersion=8.3.6`,
      headers: {
        "Host": "api.m.jd.com",
        "Origin": "https://prodev.m.jd.com",
        "User-Agent": UA,
        ...
        if (err) {
          ...
          if (typeof res == 'object') {
            if (res.code == 0 && res.data && res.data.shareUrl) {
              $.shareCode = res.data.shareUrl.match(/3wRsPts\?s=([^&]+)/) && res.data.shareUrl.match(/3wRsPts\?s=([^&]+)/)[1] || ''
...
function getUrl() {
  return new Promise(resolve => {
    const options = {
      url: `https://u.jd.com/3wRsPts?s=${$.shareCode}`,
      followRedirect:false,
      headers: {
        "Host": "u.jd.com",
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
