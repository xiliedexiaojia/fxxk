...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('http://cdn.boledao.com/shareCodes/city.json');}catch (e) {}
    if(res.length > 0){
        if(res.length > 3){
    ...
    for (let i = 0; i < cookiesArr.length && inviteCodes.length === 0; i++) {
        if (cookiesArr[i]) {
            ...
            $.nickName = '';
            message = '';
            UA = `jdapp;iPhone;10.3.0;14.6;${randomString(40)};M/5.0;network/wifi;ADID/;model/iPhone9,1;addressid/0;appBuild/167903;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1;`
            uuid = UA.split(';')[4]
            //await TotalBean();
