...
let inviteCodes = ['-ryUXvhdZWBBNhHDQorFpM2CIUgRVyFD','-ryUXKkNYWJLMReRHN7GoZpBpnpPUs-Q']
$.shareCodesArr = [];
...
function taskPostUrl(functionId,body) {
    return {
        url: `${JD_API_HOST}`,
        body: `functionId=${functionId}&appid=signed_wh5&body=${escape(JSON.stringify(body))}&client=wh5&clientVersion=1.0.0`,
        headers: {
'Host': 'api.m.jd.com',
'cookie':cookie,
'accept': 'application/json, text/plain, */*',
'origin': 'https://bunearth.m.jd.com',
'user-agent': 'jdapp;android;11.3.6;;;',
'content-type': 'application/x-www-form-urlencoded',
'referer': 'https://bunearth.m.jd.com/babelDiy/Zeus/x4pWW6pvDwW7DjxMmBbnzoub8J/index.html?inviteId=-ryUX_hYN2ERahqSTdrDoWj1_OSQ1qFZ&encryptedPin=RnFny2BYYTLQzNRP--txCZ4ba8uY8tTA1pDI&sid=328d32b6c16cd3c9e34addbc657743fw&un_area=17_1381_0_0',
...
function getInfo(inviteId, flag = false) {
    ...
    //let body ={"lbsCity":"","realLbsCity":"","inviteId":inviteId,"headImg":"","userName":"","taskChannel":"1","location":"","safeStr":"{\"log\":\"~/\",\"sceneid\":\"CHFhPageh5\",\"random\":\"\"}"}
    return new Promise((resolve) => {
        $.post(taskPostUrl("city_getHomeDatav1",body), async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.code === 0) {
                            if (data.data && data['data']['bizCode'] === 0) {
                                ...
                                console.log('自己的助力马：'+data.data.result.userActBaseInfo.inviteId)
                                ...
                                console.log(`\n\n${data.data.result.userActBaseInfo.inviteId ? '助力好友' : '获取邀请码'}失败:${data.data.bizMsg}`)
                                if (flag) {
                                    if (data.data && !data.data.result.userActBaseInfo.inviteId) {
                                        console.log(`账号已黑，看不到邀请码\n`);
                                    }
...
function getUA(){
    $.UA = `Mozilla/5.0 (iPhone; CPU iPhone OS 14_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.2(0x18000239) NetType/4G Language/zh_CN`
}
