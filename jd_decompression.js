 ...
 * 第一个号会给作者助力，其他号会给第一个号助力，活动期间貌似只有一次助力机会
 cron  15 4,16 1-16,21-30 9,10 * https://raw.githubusercontent.com/star261/jd/main/scripts/jd_decompression.js
 */
...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('http://cdn.boledao.com/shareCodes/decompression.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('http://cdn.boledao.com/shareCodes/decompression.json');}catch (e) {}
        if(!res){res = [];}
    }
